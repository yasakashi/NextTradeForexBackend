...
namespace NextTradeAPIs.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("/api/users/createwithmobile")]
        public async Task<IActionResult> SaveUser(UserRegisterWithMobileMOdel model)
        {
            StackTrace stackTrace = new StackTrace();
            string processId = Guid.NewGuid().ToString();
            string methodpath = stackTrace.GetFrame(0).GetMethod().DeclaringType.FullName + " => " + stackTrace.GetFrame(0).GetMethod().Name;
            ...
            try
            {
                ...
                message = await _userServices.SaveUserWithMobile(model.mobile, userlogin, processId, clientip, hosturl, _bearer_token);
