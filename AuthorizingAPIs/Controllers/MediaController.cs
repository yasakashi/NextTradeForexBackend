...
namespace NextTradeAPIs.Controllers
{
    ...
    public class MediaController : ControllerBase
    {
        ...
        SiteMediaFileServices _thisService;

        public MediaController(AuthorizationService authorizationService,
                                       IHttpContextAccessor httpContextAccessor,
                                       IWebHostEnvironment env,
                                       SystemLogServices systemLogServices,
                                       BlockedIPServices blockedIPServices,
                                       CategoriesServices userTypeServices,
                                       SiteMediaFileServices courseBuilderServices,
        ...
        [HttpPost]
        [Route("/api/learntotrade/addnewmedia")]
        [AllowAnonymous]
        public async Task<IActionResult> Insertmedia([FromForm] SiteMediaFileDto model)
        {
            ...
            try
            {
                ...
                if (model.mediafile != null)
                {
                    ...
                    if (model.mediafile.FileName == null || model.mediafile.FileName.Length == 0)
                    {
                    ...
                    {
                        model.filecontenttype = model.mediafile.ContentType;
                        model.filename = model.mediafile.FileName;

                        model.mediafile.CopyTo(ms);
                        message = await _thisService.SaveFile(ms.ToArray(), model, userlogin.userid, model.mediafile.FileName, sitePath, hosturl);
                    }
                    if (message.MessageCode < 0)
                        return BadRequest(message);

                    FileActionDto _fileinfo = message.MessageData as FileActionDto;
                    model.filepath = _fileinfo.filepath;
                    model.fileurl = _fileinfo.fileurl;
                ...
                message = await _thisService.AddNewSiteMediaFile(model, userlogin, processId, clientip, hosturl);
        ...
        [HttpPost]
        [HttpDelete]
        [Route("/api/learntotrade/deletemedia")]
        [AllowAnonymous]
        public async Task<IActionResult> Deletemedia(SiteMediaFileSearchDto model)
        {
            StackTrace stackTrace = new StackTrace();
            SystemMessageModel message;
            string processId = Guid.NewGuid().ToString();
            ...
            try
            {
                ...
                message = await _thisService.RemoveSiteMediaFile(model, userlogin, processId, clientip, hosturl);
        ...
        [HttpPost]
        [Route("/api/learntotrade/getmedias")]
        [AllowAnonymous]
        public async Task<IActionResult> Getmedias(SiteMediaFileSearchDto model)
        {
            StackTrace stackTrace = new StackTrace();
            SystemMessageModel message;
            string processId = Guid.NewGuid().ToString();
            ...
            try
            {
                ...
                message = await _thisService.GetSiteMediaFiles(model, userlogin, processId, clientip, hosturl);
        ...
        [HttpPost]
        [HttpPut]
        [Route("/api/learntotrade/editmedia")]
        [AllowAnonymous]
        public async Task<IActionResult> Updatemedia([FromForm] SiteMediaFileDto model)
        {
            StackTrace stackTrace = new StackTrace();
            SystemMessageModel message;
            string processId = Guid.NewGuid().ToString();
            ...
            try
            {
                ...
                message = await _thisService.EditSiteMediaFile(model, userlogin, processId, clientip, hosturl);
