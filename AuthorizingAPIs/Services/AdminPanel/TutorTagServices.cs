...
{
    public class TutorTagServices
    {
        SBbContext _Context { get; set; }
        LogSBbContext _LogContext { get; set; }
        SystemLogServices _systemLogServices;
        private readonly IConfiguration _config;
        public TutorTagServices(SBbContext context, LogSBbContext logcontext, IConfiguration config, SystemLogServices systemLogServices)
        {
            _Context = context;
        ...
        public async Task<SystemMessageModel> GetTutorTags(TutorTagDto model, UserModel? userlogin, string processId, string clientip, string hosturl)
        {
            ...
            try
            {
                IQueryable<TutorTag> query = _Context.TutorTags;

                if (!string.IsNullOrEmpty(model.name))
                    query = query.Where(x => x.name.Contains(model.name));

                if (!string.IsNullOrEmpty(model.slug))
                    query = query.Where(x => x.slug.Contains(model.slug));
                ...
                List<TutorTagDto> data = await query.Skip((pageIndex - 1) * PageRowCount).Take(PageRowCount)
                                                  .Select(x => new TutorTagDto()
                                                  {
                                                      Id = (Guid)x.Id,
                                                      name = x.name,
                                                      slug = x.slug,
        ...
        public async Task<SystemMessageModel> AddNewTutorTag(TutorTagDto model, UserModel? userlogin, string processId, string clientip, string hosturl)
        {
            SystemMessageModel message;
            StackTrace stackTrace = new StackTrace();
            string methodpath = stackTrace.GetFrame(0).GetMethod().DeclaringType.FullName + " => " + stackTrace.GetFrame(0).GetMethod().Name;
            ...
            try
            {
                ...
                await _Context.TutorTags.AddAsync(data);
        ...
        public async Task<SystemMessageModel> RemoveTutorTag(TutorTagDto model, UserModel? userlogin, string processId, string clientip, string hosturl)
        {
            SystemMessageModel message;
            StackTrace stackTrace = new StackTrace();
            string methodpath = stackTrace.GetFrame(0).GetMethod().DeclaringType.FullName + " => " + stackTrace.GetFrame(0).GetMethod().Name;
            ...
            try
            {
                ...
                TutorTag data = await _Context.TutorTags.FindAsync(model.Id);
                ...
                _Context.TutorTags.Remove(data);
        ...
        public async Task<SystemMessageModel> EditTutorTag(TutorTagDto model, UserModel? userlogin, string processId, string clientip, string hosturl)
        {
            SystemMessageModel message;
            StackTrace stackTrace = new StackTrace();
            string methodpath = stackTrace.GetFrame(0).GetMethod().DeclaringType.FullName + " => " + stackTrace.GetFrame(0).GetMethod().Name;
            ...
            try
            {
                ...
                _Context.TutorTags.Update(data);
        ...
        /*
        public async Task<SystemMessageModel> SaveFile(byte[] filecontent, TutorTagDto model, long userid, string FileName, string sitePath, string hosturl)
        {
            string filegroupname = "TutorTagfile";
            try
            {
                if (filecontent != null)
                {
